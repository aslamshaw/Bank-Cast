...
class FeatureEngineer(BaseEstimator, TransformerMixin):
    def __init__(self):
        self.duration_bins_ = None
        self.age_bins_ = None

    def fit(self, X, y=None):
        self.duration_bins_ = pd.qcut(X['duration'], q=4, retbins=True, duplicates='drop')[1]
        self.age_bins_ = [17, 30, 45, 60, 100]
        return self
    ...
    def transform(self, X):
        ...
        X_['age_group'] = pd.cut(X_['age'], bins=self.age_bins_,
                                 labels=['Young', 'Adult', 'Senior', 'Elderly'], include_lowest=True)
        X_['loan_exposure'] = (X_['housing'] == 'yes').astype(int) + (X_['loan'] == 'yes').astype(int)
        X_['contact_intensity'] = X_['campaign'] / (X_['previous'] + 1)
        X_['prev_success'] = (X_['poutcome'] == 'success').astype(int)
